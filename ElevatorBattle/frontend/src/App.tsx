...
import { SimulationState, SimulationConfig, PhaseSnapshot } from './types'; // Added PhaseSnapshot
import { 
  CONFIG_MORNING_RUSH, 
  CONFIG_LUNCH_RUSH, 
  CONFIG_END_OF_DAY,
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // FIX: Listen for phaseHistory in the final event to ensure "End of Day" is captured
    socket.on('simulation_finished', (data: { history: any[], phaseHistory: PhaseSnapshot[] }) => {
        setSimState((prevState) => {
            if (prevState) {
                // Merge the final history AND phaseHistory into the summary
                setSummary({ 
                    ...prevState, 
                    history: data.history,
                    phaseHistory: data.phaseHistory // <--- CRITICAL FIX
                }); 
            }
            return null; 
        });
